...
{
    //The inbuilt string.Split() method uses pattern matching using regex which makes is slow for the specific scenario we have here
    //where we just want to split into separate lines (pre-known separator '\r' '\n'). It also leads to a lot of allocations when the result
    //is used in a foreach loop. Using the new ReadOnlySpan, we can avoid allocations and make the operation much faster by providing a enumerator
